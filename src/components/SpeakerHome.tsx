...
const SpeakerHome = ({ products }: ProductListProps) => {
  const router = useRouter();
  const zx9 = products.find((product) =>
    product.name.toLowerCase().includes('zx9')
  );
  const ZX7 = products.find((product) =>
    product.name.toLowerCase().includes('zx7')
  );
  const YX1 = products.find((product) =>
    ...
    <>
      <div className="flex justify-center items-center gap-10 h-1/2 mx-auto w-2/3 pt-12  bg-primary rounded-sm ">
        ...
        <div className="text-white w-70 ">
          <h2 className="font-bold text-4xl mb-7">{zx9?.name.toUpperCase()}</h2>
          <h6 className="text-xs">
            Upgrade to premium speakers that are phenomenally built to deliver
            truly remarkable sound.
          </h6>
          <Button
            className="bg-black w-40 my-10 h-10"
            onClick={() => {
              router.push(`/${zx9?.category}/${zx9?.slug}`);
            }}
          >
            See product
          </Button>
      ...
      <div className="relative w-2/3 mx-auto my-10 h-100">
        ...
        <div className="flex flex-col pt-40 pl-30">
          <h2 className="font-bold text-4xl mb-7">{ZX7?.name.toUpperCase()}</h2>
          <Button
            variant="outline"
            className="bg-off-white border-1 border-black w-40  h-10"
            onClick={() => {
              router.push(`/${ZX7?.category}/${ZX7?.slug}`);
            }}
          >
            See product
          </Button>
      ...
      <div className="flex justify-center mb-10">
        ...
        <div className="bg-off-white flex flex-col ml-10 w-1/3 pl-30 justify-center rounded-sm shadow-sm hover:shadow-md transition">
          <h2 className="font-bold text-4xl mb-7">{YX1?.name.toUpperCase()}</h2>
          <Button
            variant="outline"
            className="bg-off-white border-1 border-black w-40  h-10"
            onClick={() => {
              router.push(`/${YX1?.category}/${YX1?.slug}`);
            }}
          >
            See product
          </Button>
